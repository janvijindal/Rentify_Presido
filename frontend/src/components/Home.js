  ...
  const applyFilters = () => {
    ...
    if (priceFilter !== "") {
      filteredData = filteredData.filter(
        (land) => land.price <= parseInt(priceFilter)
      );
    }

    if (areaFilter !== "") {
      filteredData = filteredData.filter((land) =>
        land.area.toLowerCase().includes(areaFilter.toLowerCase())
      );
    }
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          setLandData((prevLandData) =>
            prevLandData.map((land) => {
              if (
                likedLands.some((likedLand) => likedLand.landId === land._id)
              ) {
    ...
    <div className="mt-10">
      <div className="row d-flex justify-content-center ">
        ...
        <div className="d-flex justify-content-center">
          <button className="btn btn-primary mb-2 me-2" onClick={applyFilters}>
            Apply Filters
          </button>
          <button className="btn btn-secondary mb-2" onClick={clearFilters}>
            Clear Filters
          </button>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-10 mx-5 overflow-hidden mt-10 ">
        {filteredLandData.map((land) => {
          // Filter likeData to get likes count for the current land
          const likesCount = likeData.filter(
            (like) => like.landId === land._id
          ).length;

          return (
            <div key={land._id} className="col-md-4 mb-3">
              <div className="w-[400px] h-[580px] border border-black rounded-md    ">
                <img
                  src={home}
                ...
                <div className="card-body px-3 py-2">
                  <h5 className="card-title font-bold mb-2 ">{land.place.toUpperCase()}</h5>
                  <p className="card-text">Area: {land.area}</p>
                  <p className="card-text">Bedrooms: {land.numberOfBedrooms}</p>
                  <p className="card-text">
                    Bathrooms: {land.numberOfBathrooms}
                  </p>
                  <p className="card-text">Price: ${land.price}</p>
                  <p className="card-text">
                    Nearby Hospitals: {land.nearbyHospitals ? "Yes" : "No"}
                  </p>
                  <p className="card-text">
                    Nearby Colleges: {land.nearbyColleges ? "Yes" : "No"}
                  </p>
                  ...
                  <div className="text-end flex items-center justify-between">
                    <button
                      type="button"
                      className=" bg-blue-500 px-3 py-2 text-white rounded-md grid place-content-center me-2"
                    ...
                    <button
                      type="button"
                      className={` text-2xl  ${
                        land.liked ? "text-red-500" : "text-black"
                        
                      }`}
                      onClick={() => handleLikeClick(land)}
                    >
                      {land.liked ? <FcLike/> : <FaRegHeart/>}
                    </button>
                  </div>
                </div>
              </div>
